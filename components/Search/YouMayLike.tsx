...
export default function YouMayLike() {
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      // Get the user's most ordered drink
      const topDrinkResult: any[] = await db.getAllAsync(`
        SELECT c.id, c.type, COUNT(oh.id) as order_count
        ...
        LIMIT 1
      `);
      if (!topDrinkResult.length) return;
      const topDrink = topDrinkResult[0];
      const favType = topDrink.type;
      const excludeId = topDrink.id;
      // Get drinks of that type, excluding the most ordered drink
      const recs: any[] = await db.getAllAsync(`
        SELECT * 
        FROM Coffee
        WHERE type = ? and id <> ?
      `, [favType, excludeId]);
